...
module.exports = adapter => {
  ...
  return function odata_update(req, res, next) {
    ...
    if (!one) {
      ...
      // REVISIT: This is not the specified behavior.
      // REVISIT: See: https://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_UpdateaCollectionProperty
      // REVISIT: The expected request body is [], so re-routing based on keys from the body is not what's intended
      if (from.ref.length > 1)
        cds.error(`Method PUT is currently not supported when addressing collections via navigations!`, { status: 405 })
      ...
      for (let k in entity.keys) {
        if (data[k]) keys[k] = data[k]
        if (entity.keys[k]['@cds.on.insert']?.['='] === '$user') keys[k] = cds.context?.user?.id
        if (!keys[k]) cds.error(`All keys must be provided for ${req.method} on entity collections`, { status: 405 })
      }
