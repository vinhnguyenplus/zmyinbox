...
const _normalize = (err, req, keep,
  ...
  let key = err.message || err.code || status // REVISIT: Should we use the message over the code every time?
  if (key?.startsWith?.('{i18n>')) key = key.slice(6,-1)
  let msg = i18n.messages.at (key, '', err.args) // lookup messages for log output from factory default texts
  if (msg && msg !== key) {
    Object.defineProperty (err, 'UGLY_LEAN_DRAFT_VOODOO', { value: err.message, enumerable: false }) // REVISIT
  ...
  Object.defineProperty(err, 'toJSON', { value: function() {
    ...
    if (locale) that.message = i18n.messages.at(key, locale, err.args)
