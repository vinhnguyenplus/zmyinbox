...
const draftHandle = async function (req) {
  ...
  if (
    ...
    let result = data
    if (!isNewDraftViaActionEnabled) {
      if (Array.isArray(data)) result = data.map(d => ({ ...d, IsActiveEntity: true }))
      else result = { ...data, IsActiveEntity: true }
    }
    ...
    if (IS_PERSISTED_DRAFT_MESSAGES_ENABLED) {
      _req.on('failed', async error => {
        ...
        if (_req.errors?.length) {
          errors.push(..._req.errors.map(e => ({ ...e, message: e.UGLY_LEAN_DRAFT_VOODOO || e.message })))
        } else {
...
const Read = {
  onlyActives: async function (run, query, { ignoreDrafts } = {}) {
    ...
    const drafts = !ignoreDrafts ? await Read.complementaryDrafts(query, actives) : []
