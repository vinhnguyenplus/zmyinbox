...
const COMPAT = cds.env.features.compat_assert_not_null ? { ASSERT_MANDATORY: 'ASSERT_NOT_NULL' } : {} // REVISIT: remove with cds^10
...
class Validation {
  ...
  error (code, path, leaf, message, args) {
    ...
    err.code = COMPAT[code] || code
    err.message = typeof message === 'string' ? message : code
    ...
    if (path) {
      ...
      err.path = path.map (p => p?.row ? p.index : p)
    }
    return err
  }
