...
class ConsumptionAPI {
  ...
  get schedule() {
    function schedule (...args) {
      const req = _req4 (...args), {ms4} = cds.utils, cron = t => /^([0-9*][-0-9*/,]*\s+){4}[0-9*][-0-9*/,]*$/.exec(t)?.[0]
      return {
        after (t,u) { (req.queue ??= {}).after = ms4(t,u);            return this },
        every (t,u) { (req.queue ??= {}).every = cron(t) ?? ms4(t,u); return this },
        asTask(t)   { (req.queue ??= {}).task  = t ?? req.event;      return this },
        then: (r,e) => cds.queued(this).send(req).then(r,e)
      }
    }
    schedule.task = (function schedule_task (...args) { return this.schedule(...args).asTask() }).bind(this)
    return super.schedule = schedule
  }
  get unschedule() {
    function unschedule (task) { return cds.queued(this)._unschedule(task) }
    unschedule.task = (function unschedule_task (task) { return this.unschedule(task) }).bind(this)
    return super.unschedule = unschedule
  }
