...
module.exports = class ODataAdapter extends HttpAdapter {
  ...
  logger(req, _, next) {
    ...
    if (req.__proto__.method in { PUT: 1, PATCH: 1 }) return next()
    // req._subrequest is set for batch subrequests
    let method = req._subrequest ? '> ' + req.method : req.method
    let path = req._subrequest ? req.path : decodeURI(req.baseUrl + req.path)
    let details = Object.keys(req.query).length ? { ...req.query } : ''
    LOG.info(method, path, details)
    next()
  }
}
