...
function read (res, options) {
  try {
    const lines = CSV.parse (readFileSync (res, 'utf-8'))
    if (options === 'objects') {
      const [ header, ...rows ] = lines
      return rows.map (row => header.reduce ((o, k, i) => (o[k] = row[i], o), {}))
    }
    else return lines
  } catch (e) {
    // eslint-disable-next-line preserve-caught-error
    if (e.code !== 'ENOENT') throw new Error (`Corrupt .csv file: ${res}.csv`)
  }
}
