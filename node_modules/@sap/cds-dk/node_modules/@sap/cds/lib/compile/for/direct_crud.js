...
module.exports = function cds_compile_for_direct_crud(csn) {
  ...
  for (const each in csn.definitions) {
    ...
    if (!def['@Common.DraftRoot.NewAction'] && def['@odata.draft.enabled']) {
      ...
      if (Object.values(params).some(p => p.type === 'cds.Association' || p.type === 'cds.Composition')) {
        LOG._warn && LOG.warn(`Skipping adding draftNew action for ${each} due to association as key`)
        continue
      }
