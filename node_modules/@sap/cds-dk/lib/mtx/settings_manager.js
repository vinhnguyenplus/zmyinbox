...
async function fetchPasscodeUrl(params) {
    ...
    DEBUG?.(`Trying to get passcode URL from GET`, fetchUrl);
    if (params.has('subdomain')) {
    ...
    try {
        ...
        if (error.status === 404) {
            ...
            DEBUG?.(getMessage(`Getting passcode URL failed`, { error: netError }));
        }
        return undefined;
    }
    ...
    static async updateUrls(params, logout, loadedSettings) {
        ...
        async function updateTokenUrl() {
            ...
            const renewUrl = !logout && !params.get('skipToken')
                && (!tokenUrl || tokenUrl.includes('//-/cds') || tokenUrl.includes(OAUTH_PATH_LEGACY)
