...
async function _mergeYAML(into, from, project, semantics = {}) {
    ...
    if (relationships && Array.isArray(relationships)) {
        semantics.additions ??= []
        const add = semantics.additions

        for (const rel of relationships) {
            for (const ins of (Array.isArray(rel.insert) ? rel.insert : [rel.insert])) {
                if (!add.includes(ins)) add.push(ins)
            }
            const base = rel.into?.[0]
            if (base && !add.includes(base)) add.push(base)
        }
    }
            ...
            if (YAML.isSeq(templateNode)) {
                ...
                additions?.filter(addition => {
                        if (typeof addition.in === 'string' || Array.isArray(addition.in) && typeof addition.in[0] === 'string') return true
                            const base = Array.isArray(addition.in) ? addition.in[0] : addition.in
                            const keyPath = Array.isArray(addition.in) ? addition.in[1] : ''
                            const inEntry = entryMap.get(base)
                            if (!inEntry) return false
                            const depth = String(keyPath).split('.').filter(Boolean).length
                            const expectedParent = depth > 1 ? collectionStack[depth] : parent
                            return inEntry.node === expectedParent
                    })
                    .filter(addition => {
