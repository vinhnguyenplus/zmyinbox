 ...
 * @returns {Promise<Record<string, object[]>>} the filled data
 */
const asJson = module.exports = async (data, csn, numRecords=1, options={}) => {
  ...
  for (let [key, d] of Object.entries(data)) {
    if (contentType === 'json' && d.deleteReference && !d.keepReference) {
      delete data[key]
    }
    delete d.keepReference
...
function dataForField(element, artifact, csn, record, data, parent, depth, referenceData) {
  ...
  switch (type) {
    ...
    case 'cds.Decimal': { // Decimal(9,5), Decimal(9), Decimal(precision:5), Decimal(2,2)
      ...
      annotatedIntRange(element, range)
      if (scale === 0) return randomIntInRange(range)
      const intPart = randomIntInRange(range)
      const frac = Math.floor(randomUnit() * Math.pow(10, scale))
      return +(intPart + frac / Math.pow(10, scale)).toFixed(scale)
    }
    case 'cds.Double': {
      const range = {from: 0, to: 100}
    ...
    case 'cds.Association': {
      ...
      if (targetRecord) {
        ...
        if (targetRecords.length > 0) { // use record generated by following association
          ...
          if (parent && (parent.name != artifact.name)) data[target.name].keepReference = true
        }
      }
...
function randomDecimalInRange({ from: min, to: max }) {
  return min + randomUnit() * (max - min)
}

function randomIntInRange({ from: min, to: max }) {
  const span = Math.max(0, max - min + 1)
  return min + Math.floor(randomUnit() * span)
}
