...
module.exports = class PipelineTemplate extends require('../../plugin') {
    ...
    async run() {
        await Promise.all([
            copy(join(__dirname, 'files', '.pipeline')).to(join(cds.root, '.pipeline')),
            copy(join(__dirname, 'files', 'Jenkinsfile')).to(join(cds.root, 'Jenkinsfile'))
        ])
        ...
        if (hasUI5) { // UI5 integration might be needed even without HTML5 repo
            console.log('Adding UI5 integration to the pipeline for the detected UI5 apps...')
