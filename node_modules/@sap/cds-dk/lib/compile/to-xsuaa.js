...
module.exports = function compile_to_xsuaa (model, options) {
  ...
  const authAnnotations = def => {
    if (!def) return

    let reqAnnotations = def['@requires']
  ...
  cds.reflect(model).foreach(def => {
    authAnnotations(def)
    if (def.actions)   Object.values(def.actions).forEach(authAnnotations)
    if (def.functions) Object.values(def.functions).forEach(authAnnotations)
  })
