...
module.exports = class HelmBuildPlugin extends require('../plugin') {
  ...
  static hasTask() {
    if (exists('chart/Chart.yaml')) {
      ...
      const chartYaml = yaml.parse(chartFile)
  ...
  async build() {
    if (!HelmBuildPlugin.hasTask()) {
      this.pushMessage("Helm Chart not found. Execute `cds add kyma` to add the helm chart.", SEVERITY_WARNING)
      return
    }

    const project = readProject()
    const { hasApprouter, hasConnectivity, hasHtml5Repo } = project
