...
const { BuildError, relativePaths, BuildMessage, hasOptionValue, getWorkspacePaths, normalizePath } = require('../../util')
...
module.exports = class HanaBuildPlugin extends require('../internal') {
    ...
    async build() {
        ...
        if (!cds.cli.options.dry) {
            ...
            // cds10: disabled as it did not work with MTX domain concept which allows partial app deployments
            // consider enabling this in cds10
            ...
            if (this.isStagingBuild()) {
                const relocatedDirs = await this._relocateSynonymConfigsToCfg()
     ...
     */
    async _relocateSynonymConfigsToCfg() {
        const destSrcDir = path.join(this.task.dest, 'src')
        const destCfgDir = path.join(this.task.dest, 'cfg')

        const synonymConfigFiles = await find(destSrcDir, { filter: (entry) => {
            if (isdir(entry)) {
                return true
            }
