...
const { FLAVOR_LOCALIZED_EDMX } = require('../constants')
const DEBUG = cds.debug('cli|build')

module.exports = class EdmxBuildPlugin extends require('./featureToggles') {

    async compileToEdmx(model, edmxDest, compileOptions = {}) {
        const promises = []

        // generate edmx for services only that have the odata protocol
        const serviceNames = cds.reflect(model).services
            .filter(service => 'odata' in service.protocols)
    ...
    _writeEdmxForLang(content, serviceName, lang, edmxDest) {
        ...
        if (this._cachedEdmx) this._cachedEdmx.set(fileName, content)
        if (edmxDest) {
