...
module.exports = ({ cwd = process.cwd(), script, ext, ignore=RegExp(), watched, env={}, delay=200, options={} })=>{
  ...
  const restart = _coalesceEvents(delay, (eventsAndOpts) => {
    ...
    _kill (child, () => {
      ...
      // env.CDS_WATCH_HOT_RELOAD = options['experimental-hot-reload']
  ...
  const watcher = watch (watched,{ filter, ignore, hotReload: options['experimental-hot-reload'] })
  if (DEBUG) watcher.on('close', ()=> DEBUG('⚡️', 'cds watch - file watcher closed'))
  watcher.on('change', restart)
  watcher.on('refreshModel', _coalesceEvents(delay, (eventsAndOpts) => {
    child.send({ code:'refreshModel', eventsAndOpts })
  }))
